...
**Catalog reference for the stretch goal:** `references/pvc-fittings-reference.md` — a user-supplied PVC fitting catalog (shapes, materials, installation positions, reference photos) with per-fitting Three.js primitive decompositions. The recipes come from two user-supplied working Three.js prototypes (open them in a browser to inspect each shape): `references/pvc-fittings-3d-viewer.html` covers all 20 water/drainage catalog fittings (torus-arc elbows, bell-socket rings, brass-insert threaded fittings, wye, P-trap, cleanout, flanges, pipe clamp, ball valve), and `references/electric-conduit-3d-viewer.html` covers the electric system (conduit cutaways with colored L/N/PE wire cores, junction box with Wago connectors, MCB distribution board, switch, socket, cable crimp lug, saddle clip). Use them when upgrading from the schematic v1 shapes to catalog-accurate fittings; the main new requirement is that `computeMepJoints` must also expose the segment direction vectors at each joint so elbows can be oriented.

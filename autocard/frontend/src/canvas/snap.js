...
/**
 * Find the nearest snap point for a given cursor position.
 * @param {Array} elements - All drawing elements
 * @param {object} pt - Cursor position {x, y}
 * @param {object} snapModes - Active snap modes {endpoint, midpoint, center, grid, intersection}
 * @param {number} gridSize - Grid spacing
 * @param {number} threshold - Snap distance threshold in canvas units
 * @returns {{point: {x,y}, type: string} | null}
 */
export function findNearestSnap(elements, pt, snapModes, gridSize = 40, threshold = 10) {
  let best = null;
  let bestDist = threshold;

  const checks = [];
  if (snapModes.endpoint) checks.push(snapEndpoint);
  if (snapModes.midpoint) checks.push(snapMidpoint);
  if (snapModes.center) checks.push(snapCenter);
  if (snapModes.intersection) checks.push(snapIntersection);

  for (const check of checks) {
    const result = check(elements, pt, bestDist);
  ...
  // Grid snap last (lower priority)
  if (snapModes.grid) {
    const result = snapGrid(pt, gridSize, bestDist);
    if (result) {
      best = result;
    }
  }

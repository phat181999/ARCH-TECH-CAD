...
function elementToMesh(el: any, blockDefs: any) {
  if (!el) return null;
  ...
  if (el.type === "block") {
    ...
    return (
      ...
      >
        {blockDef.elements.map((be: any) => elementToMesh(be, blockDefs))}
      </group>
    );
  }
...
function Scene({ elements, blockDefs, selectedIds, onSelect }: { elements: any[]; blockDefs: any; selectedIds?: string[]; onSelect?: (id: string) => void }) {
...
export default function ThreeViewer({ elements, blockDefs, visible }: { elements: any[]; blockDefs: any; visible: boolean }) {

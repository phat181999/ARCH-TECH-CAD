...
interface CloudProvider {
  ...
  appKey?: string;
  redirectUri: string;
  authUrl: (key: string, redirectUri: string) => string;
}
...
const CLOUD_PROVIDERS: CloudProvider[] = [
  ...
  },
  {
    ...
    icon: "📦",
    color: "#0061FF",
    appKey: (import.meta as any).env.VITE_DROPBOX_APP_KEY || "",
    redirectUri: `${window.location.origin}/auth/dropbox/callback`,
    authUrl: (appKey: string, redirectUri: string) =>
...
interface CloudStorageProps {
  onImportDrawing?: (file: CloudFile) => void;
  onExportDrawing?: () => void;
}

export default function CloudStorage({ onImportDrawing, onExportDrawing }: CloudStorageProps) {
  ...
  const handleImport = (file: CloudFile) => {
    if (onImportDrawing) {
